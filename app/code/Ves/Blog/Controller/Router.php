...
class Router implements RouterInterface
{
    ...
    public function match(RequestInterface $request)
    {
        ...
        if (!$this->dispatched) {
            ...
            // LATEST PAGE
            if(count($urlKeys)==2 && $urlPrefix == $urlKeys[0] && $this->endsWith($urlKeys[1], $urlSuffix)){
                if(isset($urlKeys[1]) && (str_replace($urlSuffix, "", $urlKeys[1]) == $latestPageRoute) ){
                    $request->setModuleName('vesblog')
                    ->setControllerName('latest')
                    ->setActionName('view');
                    $request->setAlias(\Magento\Framework\Url::REWRITE_REQUEST_PATH_ALIAS, $origUrlKey);
            ...
            if(count($urlKeys)==3 && $urlPrefix == $urlKeys[0] && $urlKeys[1]=='author'){
                $alias = str_replace($urlSuffix, "", $urlKeys[2]);
                $user = $this->_user->create()->getCollection()
                ->addFieldToFilter('username', $alias)
                ->getFirstItem();
                $this->_coreRegistry->register("author", $user);
                if($user){
                    $request->setModuleName('vesblog')
